...
export async function seedDatabase(dataSource: DataSource) {
  const identificationTypeRepository =
    dataSource.getRepository(IdentificationType);
  ...
  const cedulaType = await identificationTypeRepository.findOne({
    where: { code: 'CC' },
  });
  ...
  if (superAdminRole && cedulaType) {
    ...
    console.error(
      'No se encontró el rol SUPERADMINISTRADOR o el tipo de identificación CC.',
    );
  }
}
